...
FROM hub.anduin.com/node:21-alpine AS npm-env
ARG CSPROJ_PATH
WORKDIR /src
COPY . .
...
FROM hub.anduin.com/anduin/internalimages/dotnet AS build-env
ARG CSPROJ_PATH
ARG PROJ_NAME
WORKDIR /src
...
FROM hub.anduin.com/anduin/internalimages/dotnet
ARG PROJ_NAME
WORKDIR /app
